...
public class Test_002 : MonoBehaviour
{
    ...
    void Start()
    {
        int answer;                          //변수 in answer 선언
        answer = 1 + 2;                      //answer에 1 + 2 값 입력
        Debug.Log(answer);                   //Console.log() answer 값을 보여줌

        int n1 = 8;                          //변수 int 선언 이름른 n1이고 8의 값을 입력
        int n2 = 9;                          //변수 int 선언 이름른 n1이고 9의 값을 입력
        ...
        answer2++;                           //answer2 = answer2 + 1의 축약 보통 번복문에서 1씩 더해서 처리할때 쓰임
        Debug.Log(answer2);                  //Console.log() answer2 값을 보여줌
    }
