...
public class RaycastController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            Ray cast = Camera.main.ScreenPointToRay(Input.mousePosition);           //화면에서의 2차원 마우스 좌표를 임력해서 3D 카메라에서 Ray를 만드는 함수

            RaycastHit hit; 

            if(Physics.Raycast(cast, out hit))      //Rast 가 충돌 감지된것을 hit 로 돌려준다. 
            {
